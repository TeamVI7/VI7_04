...
public static class CutsceneGizmos
{
    ...
    /// <summary>The lofted parabola the missile actually flies: a straight
    /// interpolation with 4t(1-t) added on Y, sampled. Matches
    /// MissileFlightCutscene.FlyToTarget exactly so what you see is the run.</summary>
    [System.Diagnostics.Conditional("UNITY_EDITOR")]
    public static void Arc(Vector3 start, Vector3 end, float arcHeight, Color color,
                           int samples = 48)
    {
        if (samples < 2) return;
    ...
    /// <summary>Point on that same parabola, for marking the cut point or a
    /// chase-camera sample without re-deriving the curve at the call site.</summary>
    public static Vector3 ArcPoint(Vector3 start, Vector3 end, float arcHeight, float t)
    {
